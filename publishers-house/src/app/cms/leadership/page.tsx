...
const S = {
  ...
  Paper200: "#E8ECF7", Paper300: "#D3DAEC",
  White: "#FFFFFF", Slate500: "#747CA1",
...
const empty = { name: "", role: "", bio: "", photoUrl: "", order: 0, published: false };
...
export default function LeadershipEditor() {
  ...
  const handleEdit = (item: any) => {
    setForm({ name: item.name || "", role: item.role || "", bio: item.bio || "", photoUrl: item.photoUrl || "", order: item.order || 0, published: item.published || false });
  ...
  const togglePublish = async (item: any) => {
    await updateDoc(doc(db, "leadership", item.id), { published: !item.published });
    fetchData();
  };

  const F = (field: string, value: any) => setForm((f: any) => ({ ...f, [field]: value }));
  ...
  return (
    <div style={{ maxWidth: "900px", margin: "0 auto", padding: "40px 24px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "32px" }}>
        <h1 style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: "28px", color: S.Navy, margin: 0 }}>Leadership</h1>
      ...
      <div style={{ backgroundColor: S.White, border: `1px solid ${S.Paper300}`, borderRadius: "8px", padding: "28px", marginBottom: "40px" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            <button type="submit" disabled={loading} style={{ ...S.btn(S.Blue700, S.White), opacity: loading ? 0.6 : 1 }}>{loading ? "Saving..." : editingId ? "Update Leader" : "Add Leader"}</button>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        {items.length === 0 && <p style={{ fontFamily: "'Poppins', sans-serif", color: S.Slate500, textAlign: "center", padding: "40px 0" }}>No leaders yet. Add your first one above.</p>}
        {items.map(item => (
          <div key={item.id} style={{ backgroundColor: S.White, border: `1px solid ${S.Paper300}`, borderRadius: "8px", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              {item.photoUrl && <img src={item.photoUrl} alt={item.name} style={{ width: "48px", height: "48px", borderRadius: "50%", objectFit: "cover" }} />}
              <div>
                <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: "15px", color: S.Navy }}>{item.name}</div>
                <div style={{ fontFamily: "'Poppins', sans-serif", fontSize: "12px", color: S.Slate500, marginTop: "2px" }}>#{item.order} · {item.role}</div>
              </div>
            </div>
